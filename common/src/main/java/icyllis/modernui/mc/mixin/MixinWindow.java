...
public abstract class MixinWindow {
    ...
    private void onSetGuiScale(int scaleFactor, CallbackInfo ci) {
        ...
        if (monitor != null) {
            ...
            // MC 26.3: Monitor is a record that exposes the physical size via w()/h(),
            // replacing the removed GLFW.glfwGetMonitorPhysicalSize call.
            int w = monitor.w(), h = monitor.h();
            if (w > 0 && h > 0) {
                VideoMode mode = monitor.currentMode();
                metrics.xdpi = 25.4f * mode.getWidth() / w;
                metrics.ydpi = 25.4f * mode.getHeight() / h;
            }
        }
