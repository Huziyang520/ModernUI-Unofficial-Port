 ...
 * <p>
 * This class links blaze3d Vulkan internals, so it must only be touched when the game really
 * runs on its built-in Vulkan backend.
 */
@ApiStatus.Internal
public final class VanillaVulkanIntegration {
    ...
    public static VulkanBackendContext wrapContext() {
        ...
        warnIfWindowIsNotVulkanCompatible();
    ...
    /**
     * Reports the NeoForge early loading window bug that makes Minecraft 26.2 abort a few seconds
     * later with {@code GLFW error 65540: Vulkan: Window surface creation requires the window to
     * have the client API set to GLFW_NO_API}.
     * <p>
     * NeoForge's early loading screen creates the game window with an OpenGL client API (that
     * window is then adopted by Minecraft), so Minecraft's own window hints - which would ask for
     * {@code GLFW_NO_API} when the Vulkan backend is selected - never apply. The failure happens
     * inside {@code Minecraft#<init>}, far away from ModernUI, and is therefore easily mistaken
     * for a mod conflict. This is a loader bug (neoforged/NeoForge#3230), fixed upstream by
     * skipping the early loading screen when Vulkan is active (see NeoLoading-Vulkan-Fix).
     */
    private static void warnIfWindowIsNotVulkanCompatible() {
        try {
            final long handle = Minecraft.getInstance().getWindow().handle();
            final int clientApi = GLFW.glfwGetWindowAttrib(handle, GLFW.GLFW_CLIENT_API);
            if (clientApi != GLFW.GLFW_NO_API) {
                LOGGER.error(MARKER, "The game window was created with an OpenGL client API (0x{}), "
                                + "but Minecraft selected its built-in Vulkan backend. Minecraft "
                                + "will fail to create the Vulkan window surface in a moment "
                                + "(GLFW error 65540), so the game cannot start. This is NeoForge's "
                                + "early loading window bug, not a ModernUI bug - disable NeoForge's "
                                + "early loading screen or install NeoLoading-Vulkan-Fix.",
                        Integer.toHexString(clientApi));
            }
        } catch (Throwable t) {
            LOGGER.debug(MARKER, "Could not query the window's client API", t);
        }
    }

    private static VulkanDevice resolveBackend(GpuDevice device) {
        final GpuDeviceBackend backend = ((AccessGpuDevice) device).getBackend();
