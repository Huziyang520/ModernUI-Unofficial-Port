...
public abstract class TextRenderType {
    ...
    private static RenderPipeline.Builder copyOf(@Nonnull RenderPipeline base) {
        ...
        // MC 26.3: RenderPipeline no longer exposes getVertexShader()/getFragmentShader();
        // the shaders are stored in a ShaderType -> Identifier map instead.
        var shaders = base.getShaders();
        var vertexShader = shaders.get(com.mojang.renderpearl.api.pipeline.ShaderType.VERTEX);
        if (vertexShader != null) {
            builder.withVertexShader(vertexShader);
        }
        var fragmentShader = shaders.get(com.mojang.renderpearl.api.pipeline.ShaderType.FRAGMENT);
        ...
        // MC 26.3: these getters return List instead of an array
        var targets = base.getColorTargetStates();
        for (int i = 0; i < targets.size(); i++) {
            builder.withColorTargetState(i, targets.get(i));
        }
        ...
        for (int i = 0; i < formats.size(); i++) {
            builder.withVertexBinding(i, formats.get(i));
        }
